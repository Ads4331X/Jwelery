...
export default function AdminForgotPassword() {
  ...
  const handleResend = async () => {
    ...
    setError(null);
    setLoading(true);
    const res = await adminForgotPasswordRequest(normalized);
    setLoading(false);
  ...
  // Local countdown timer while the user waits to resend OTP
  useEffect(() => {
    if (!resendDisabled) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === "verify") {
      ...
      setLoading(true);
      const res = await adminForgotPasswordVerify(normalized, code);
      setLoading(false);
